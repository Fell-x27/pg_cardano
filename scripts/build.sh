...
for PG_KEY in "${!PG_CONFIGS[@]}"; do
  ...
  if [ -f "$PG_CONFIG" ]; then
    # получаем версию PostgreSQL: например, 16.2 → pg16
    RAW_VERSION=$("$PG_CONFIG" --version | grep -oE '[0-9]+\\.[0-9]+' | head -n1)
    MAJOR_VERSION="${RAW_VERSION%%.*}"  # до точки
    PG_VERSION="pg${MAJOR_VERSION}"
