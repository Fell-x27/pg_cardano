...
mod tests {
    ...
    fn test_cbor_enc_jsonb_with_hex() {
        let original_json = pgrx::JsonB(serde_json::json!({
            ...
            "hex": "0x6648d73a09b282c120c8476789f8232b7eead94ff560917ae8fc4eb1"
        }));
    ...
    #[pg_test]
    fn test_cbor_dec_to_jsonb_no_hex_detection() {
        let original_json = pgrx::JsonB(serde_json::json!({
            "ada": "is amazing!",
            "version": 1.0,
            "features": [
        ...
        let cbor_bytes = hex::decode("a4636164616b697320616d617a696e672163686578783836363438643733613039623238326331323063383437363738396638323332623765656164393466663536303931376165386663346562316776657273696f6ef93c006866656174757265738267736369656e636568617070726f616368")
            .expect("Failed to decode hex");
    ...
    #[pg_test]
    fn test_cbor_dec_to_jsonb_hex_detection() {
        let original_json = pgrx::JsonB(serde_json::json!({
            "ada": "is amazing!",
            "version": 1.0,
            "features": [
        ...
        let cbor_bytes = hex::decode("a4636164616b697320616d617a696e672163686578581c6648d73a09b282c120c8476789f8232b7eead94ff560917ae8fc4eb16776657273696f6ef93c006866656174757265738267736369656e636568617070726f616368")
            .expect("Failed to decode hex");
        let result = crate::cardano::cbor_decode_jsonb_mark_hex(&cbor_bytes);
    ...
    fn test_cbor_dec_to_json() {
        ...
        let expected_str =
            serde_json::to_string(&original_json).expect("Failed to serialize expected JSON");
